...
class BlockChain(object):
    ...
    def set_neighbours(self):
        self.neighbours = utils.find_neighbours(
            utils.get_host(
            ), self.port, NEIGHBOURS_IP_RANGE_NUM[0], NEIGHBOURS_IP_RANGE_NUM[1], BLOCKCHAIN_PORT_RANGE[0], BLOCKCHAIN_PORT_RANGE[1]
        )
    ...
    def valid_proof(self, transactions, previous_hash, nonce, difficulty=MINNING_DIFFICULTY):
    ...
    def mining(self):
        ...
        self.add_transaction(sender_blockchain_address=MINING_SENDER,
                             recipient_blockchain_address=self.blockchain_address, value=MINING_REWORD)
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if not self.valid_proof(
                block['transactions'], block['previous_hash'], block['nonce'], MINNING_DIFFICULTY
            ):
                return False
    ...
    def resolve_conflicts(self):
        ...
        logger.info({'action': 'resolve_conflicts',
                     'status': 'not_replaced'})
